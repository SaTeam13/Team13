...
public class G13Application {
	...
	ApplicationRunner init(GenderRepository genderRepository, ProvinceRepository provinceRepository,
		...
		return args -> {
			...
			custype.setCustomertypename("Normal (1,200 bath)");
			custype.setPrice(1200);
			
			customerTypeRepository.save(custype);
			
			 
			TimeRange timer = new TimeRange();
			timer.setTimerangename("3 months");
			timer.setTime(3);
			
			timeRangeRepository.save(timer);
